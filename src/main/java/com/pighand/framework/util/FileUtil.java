package com.pighand.framework.util;

import com.pighand.framework.exception.ThrowException;
...
public class FileUtil {
     ...
     * @return List<String>
     */
    public static List<String> iterator(
            String path, String queryType, String returnType, boolean isSub) {
        ...
        for (int i = 0; i < files.length; i++) {
            // 根据queryType判断是否读取
            boolean isRead =
                    ("folder".equals(queryType) && files[i].isDirectory())
            ...
            if (isSub && files[i].isDirectory()) {
                fileNames.addAll(iterator(files[i].getPath(), queryType, returnType, isSub));
            }
        }
